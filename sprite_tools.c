...
float	calc_sp_x_start(player plr, data info, crd sprite)
{
	...
	sp_angle = normA(atan2((sprite.y)- plr.y, (sprite.x)- plr.x) * (180 / M_PI));
	plr.rotationA = (int)normA(plr.rotationA - 30);
	sp_angle = sp_angle - plr.rotationA;
	sp_angle = normA(sp_angle);
